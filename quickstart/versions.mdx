...
<Steps>
  ...
  Now let's create a file called `get_version_details.py` and add the following code block to it to get version details of a router
  <Step title="Get Version Details">
    ```python get_version_details.py
    from config import API_URL, ORG_ID, HEADERS
    import requests
